...
func (s *Store) ListICloudHMESourceAccounts(ctx context.Context) ([]ICloudHMESourceAccount, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT id, name, apple_id_email, icloud_email, host,
		       ...
		       status, status_reason, alias_total, last_validated_at, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&account.LastValidatedAt, &account.CreatedAt, &account.UpdatedAt,
		); err != nil {
...
func (s *Store) GetICloudHMESourceCredentials(ctx context.Context, id int64) (ICloudHMESourceCredentials, error) {
	...
	`, id).Scan(
		...
		&credentials.LastValidatedAt, &credentials.CreatedAt, &credentials.UpdatedAt,
	)
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s *Store) SaveICloudHMECookies(ctx context.Context, id int64, cookies map[string]string, status, reason string) error {
	...
	result, err := s.pool.Exec(ctx, `
		...
		WHERE id = $1
	`, id, encrypted, status, strings.TrimSpace(reason))
	if err != nil {
...
func (s *Store) UpdateICloudHMESourceStatus(ctx context.Context, id int64, status, reason string, aliasTotal *int, validated bool) error {
	result, err := s.pool.Exec(ctx, `
		...
		SET status = $2,
		    ...
		    updated_at = now()
		WHERE id = $1
	`, id, status, strings.TrimSpace(reason), aliasTotal, validated)
	if err != nil {
...
func (s *Store) ListICloudHMEAliases(ctx context.Context) ([]ICloudHMEAlias, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&alias.AnonymousID, &alias.Label, &alias.Active, &alias.Group,
			&alias.Remark, &alias.MailReady, &alias.CreatedAt, &alias.UpdatedAt,
		); err != nil {
	...
	for _, input := range inputs {
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO icloud_hme_aliases (
				email, source_account_id, anonymous_id, label, active, group_id, created_at
			)
			...
			ON CONFLICT (email) DO UPDATE SET
				...
				updated_at = now()
		`, email, sourceID, strings.TrimSpace(input.AnonymousID), strings.TrimSpace(input.Label), input.Active, groupID, createdAt); err != nil {
